...
Respond with strict JSON: {"found": [{"index": 0, "producer": "<tool slug>", "reason": "short reason"}, ...]} -- omit indices with no plausible producer.`;
    ...
    try {
      ...
      const parsed = JSON.parse(resp.choices[0]?.message?.content ?? "{}");
      const results: { index: number; producer: string; reason: string }[] = parsed.found ?? [];
      raw.push({ batch: i / BATCH_SIZE, items, results });

      for (const r of results) {
        const item = batch[r.index];
        if (!item || r.producer === item.tool) continue;
        found.push({ from: r.producer, to: item.tool, label: item.param, confidence: "low" });
      }
    } catch (err) {
